...
`circlePlotData` function, and then use ggplot to display the layout, labelling 
the circles to show their order of placement:

```{r fig.width=4, fig.height=4} 

dat.gg <- circlePlotData(packing, npoints=50)
...
npts <- 50 
dat.gg <- circlePlotData(packings, npoints = npts) 
...
dat1 <- circlePlotData(packing1)

# area: big to small
packing2 <- circleProgressiveLayout( rev(areas) ) 
dat2 <- circlePlotData(packing2)
...
The following example shows how to display the abundance values as circles filled with the specified colours. It relies on the fact that the `id` column in the output of `circlePlotData` maps to the row number of the input data.
...
gg <- ggplot(data = dat.gg) +
  ...
  labs(title = "Hover over circle to display taxon") +
